...
--*/
{
   ...
   if (g_Battle.rgEnemy[wEnemyIndex].wObjectID != 0)
   {
      ...
      //
      // Draw HP bar below the enemy sprite
      //
      {
         SDL_Rect        barRect;
         WORD            wMaxHP = g_Battle.rgEnemy[wEnemyIndex].wMaxHP;
         WORD            wCurHP = g_Battle.rgEnemy[wEnemyIndex].e.wHealth;
         int             barWidth = 38;
         int             barHeight = 4;
         int             barX = PAL_X(g_Battle.rgEnemy[wEnemyIndex].pos) - barWidth / 2;
         int             barY = PAL_Y(g_Battle.rgEnemy[wEnemyIndex].pos) + 10;
         ...
         barRect.w = barWidth;
         barRect.h = barHeight;
         SDL_FillRect(lpDstSurface, &barRect, 0x11);

         //
         // Fill (green when HP > 30%, yellow when 15-30%, red when < 15%)
         //
         if (wMaxHP > 0)
         {
            int fillWidth = (barWidth - 2) * wCurHP / wMaxHP;
            if (fillWidth > 0)
            {
               BYTE bColor;
               if ((wCurHP * 100 / wMaxHP) > 30)
                  bColor = 0x2C;  // green
               else if ((wCurHP * 100 / wMaxHP) > 15)
                  bColor = 0x5B;  // yellow
               else
                  bColor = 0x4A;  // red
               ...
               barRect.y = barY + 1;
               barRect.w = fillWidth;
               barRect.h = barHeight - 2;
               SDL_FillRect(lpDstSurface, &barRect, bColor);
            }
         }
      }
   }
...
--*/
{
   ...
   for (i = 0, j = 0; j < MAX_ENEMIES_IN_TEAM; j++)
   {
      ...
      if (w != 0)
      {
          ...
          g_Battle.rgEnemy[i].wTrueMaxHP = g_Battle.rgEnemy[i].e.wHealth;
   ...
   //
   // Bestiary: record enemy encounters
   //
   for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
      WORD oid = g_Battle.rgEnemy[i].wObjectID;
      if (oid != 0) {
         BOOL found = FALSE;
         for (int bi = 0; bi < gpGlobals->iBestiaryCount; bi++) {
            if (gpGlobals->rgBestiary[bi] == oid) { found = TRUE; break; }
         }
         if (!found && gpGlobals->iBestiaryCount < MAX_BESTIARY_ENTRIES) {
            gpGlobals->rgBestiary[gpGlobals->iBestiaryCount++] = oid;
         }
      }
   }
   ...
   //
   if (g_wBossRushScale > 100) {
      for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
         if (g_Battle.rgEnemy[i].wObjectID != 0) {
            DWORD _s = g_wBossRushScale;
            // Scale HP: percentage of base
            g_Battle.rgEnemy[i].e.wHealth = (WORD)((DWORD)g_Battle.rgEnemy[i].e.wHealth * _s / 100);
            g_Battle.rgEnemy[i].wMaxHP = g_Battle.rgEnemy[i].e.wHealth;
            g_Battle.rgEnemy[i].wTrueMaxHP = g_Battle.rgEnemy[i].e.wHealth;
            // Scale attack/defense
            g_Battle.rgEnemy[i].e.wAttackStrength = (WORD)((DWORD)g_Battle.rgEnemy[i].e.wAttackStrength * _s / 100);
            g_Battle.rgEnemy[i].e.wMagicStrength = (WORD)((DWORD)g_Battle.rgEnemy[i].e.wMagicStrength * _s / 100);
            g_Battle.rgEnemy[i].e.wDefense = (WORD)((DWORD)g_Battle.rgEnemy[i].e.wDefense * _s / 100);
            // Speed boost (smaller effect)
            g_Battle.rgEnemy[i].e.wDexterity = (WORD)((DWORD)g_Battle.rgEnemy[i].e.wDexterity * (100 + (_s - 100) / 2) / 100);
         }
      }
   }
   ...
   if (i == kBattleResultWon)
   {
      ...
      //
      // Bestiary: increment kill counts for all defeated enemies
      //
      for (w = 0; w <= g_Battle.wMaxEnemyIndex; w++) {
         if (g_Battle.rgEnemy[w].wObjectID != 0 && g_Battle.rgEnemy[w].e.wHealth == 0) {
            WORD oid = g_Battle.rgEnemy[w].wObjectID;
            for (int bi = 0; bi < gpGlobals->iBestiaryCount; bi++) {
               if (gpGlobals->rgBestiary[bi] == oid) {
                  gpGlobals->rgBestiaryKills[bi]++;
                  break;
               }
            }
         }

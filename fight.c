...
--*/
{
   ...
   for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
   {
      ...
      g_Battle.rgEnemy[i].wMaxHP = g_Battle.rgEnemy[i].e.wHealth;
   }
...
//
// Smart AI auto-battle decision
//
static WORD PAL_AIFindItem(BOOL needUsable) {
   // Scan inventory for consumable items usable on allies or throwable at enemies.
   // Prefer items with more quantity (common items) over rare ones.
   WORD bestItem = 0;
   int bestQty = 0;
   for (int i = 0; i < MAX_INVENTORY; i++) {
      WORD item = gpGlobals->rgInventory[i].wItem;
      if (item == 0) continue;
      int qty = gpGlobals->rgInventory[i].nAmount;
      if (qty <= 0) continue;
      WORD flags = gpGlobals->g.rgObject[item].item.wFlags;
      if ((flags & kItemFlagConsuming) && (needUsable ? (flags & kItemFlagUsable) : (flags & kItemFlagThrowable))) {
         if (qty > bestQty) { bestQty = qty; bestItem = item; }
      }
   }
   return bestItem;
}

static int PAL_AIGetAllyHealth(int allyIndex) {
   return gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[allyIndex].wPlayerRole];
}

static void PAL_AIActionMagic(WORD pi, WORD mid, int target) {
   g_Battle.rgPlayer[pi].action.ActionType = kBattleActionMagic;
   g_Battle.rgPlayer[pi].action.wActionID = mid;
   g_Battle.rgPlayer[pi].action.sTarget = target;
   g_Battle.UI.iSelectedIndex = target;
   g_Battle.UI.wObjectID = mid;
   g_Battle.UI.wActionType = kBattleActionMagic;
}

static void PAL_AIActionAttack(WORD pi, int target) {
   g_Battle.rgPlayer[pi].action.ActionType = kBattleActionAttack;
   g_Battle.rgPlayer[pi].action.wActionID = 0;
   g_Battle.rgPlayer[pi].action.sTarget = target;
   g_Battle.UI.iSelectedIndex = target;
   g_Battle.UI.wActionType = kBattleActionAttack;
}

static void PAL_AIActionUseItem(WORD pi, WORD item, int target) {
   g_Battle.rgPlayer[pi].action.ActionType = kBattleActionUseItem;
   g_Battle.rgPlayer[pi].action.wActionID = item;
   g_Battle.rgPlayer[pi].action.sTarget = target;
   g_Battle.UI.iSelectedIndex = target;
   g_Battle.UI.wObjectID = item;
   g_Battle.UI.wActionType = kBattleActionUseItem;
}

static void PAL_AIActionDefend(WORD pi) {
   g_Battle.rgPlayer[pi].action.ActionType = kBattleActionDefend;
   g_Battle.rgPlayer[pi].action.wActionID = 0;
   g_Battle.rgPlayer[pi].action.sTarget = -1;
   g_Battle.UI.wActionType = kBattleActionDefend;
}

static void PAL_AIActionFlee(WORD pi) {
   g_Battle.rgPlayer[pi].action.ActionType = kBattleActionFlee;
   g_Battle.rgPlayer[pi].action.wActionID = 0;
   g_Battle.rgPlayer[pi].action.sTarget = -1;
   g_Battle.UI.wActionType = kBattleActionFlee;
}

static WORD PAL_AISelectHealMagic(WORD r, WORD mp, int *outCost) {
   // Find best healing magic that we can afford.
   // Prefer multi-target (party heal) for emergencies.
   WORD bestMid = 0;
   int bestCost = 9999;
   if (outCost) *outCost = 9999;
   for (int i = 0; i < MAX_PLAYER_MAGICS; i++) {
      WORD mid = gpGlobals->g.PlayerRoles.rgwMagic[r][i];
      if (mid == 0) break;
      WORD fl = gpGlobals->g.rgObject[mid].magic.wFlags;
      if ((fl & kMagicFlagUsableInBattle) && !(fl & kMagicFlagUsableToEnemy)) {
         WORD magicNum = gpGlobals->g.rgObject[mid].magic.wMagicNumber;
         WORD cost = gpGlobals->g.lprgMagic[magicNum].wCostMP;
         WORD type = gpGlobals->g.lprgMagic[magicNum].wType;
         if (mp >= cost && (type == kMagicTypeApplyToPlayer || type == kMagicTypeApplyToParty || type == kMagicTypeTrance)) {
            bestMid = mid;
            bestCost = cost;
         }
      }
...
static WORD PAL_AISelectAttackMagic(WORD r, WORD mp, BOOL preferAOE, int *outCost) {
   // Find best attack magic we can afford.
   // preferAOE: prefer multi-target magic when fighting many enemies.
   WORD bestMid = 0;
   int bestCost = 9999;
   if (outCost) *outCost = 9999;
   for (int i = 0; i < MAX_PLAYER_MAGICS; i++) {
      ...
      if ((fl & kMagicFlagUsableInBattle) && (fl & kMagicFlagUsableToEnemy)) {
         ...
         if (mp >= cost) {
            if (preferAOE &&
               (type == kMagicTypeAttackAll || type == kMagicTypeAttackWhole || type == kMagicTypeAttackField)) {
               bestMid = mid;
               bestCost = cost;
            } else if (!preferAOE || bestMid == 0) {
               bestMid = mid;
               bestCost = cost;
            }
         }
...
static BOOL PAL_AIHasOffensiveMagic(WORD r) {
   // Quick check if this player has any attack magic at all
   for (int i = 0; i < MAX_PLAYER_MAGICS; i++) {
      WORD mid = gpGlobals->g.PlayerRoles.rgwMagic[r][i];
      if (mid == 0) break;
      WORD fl = gpGlobals->g.rgObject[mid].magic.wFlags;
      if ((fl & kMagicFlagUsableInBattle) && (fl & kMagicFlagUsableToEnemy)) return TRUE;
   }
   return FALSE;
}

static BOOL PAL_AIHasHealMagic(WORD r) {
   // Quick check if this player has any healing magic
   for (int i = 0; i < MAX_PLAYER_MAGICS; i++) {
      WORD mid = gpGlobals->g.PlayerRoles.rgwMagic[r][i];
      if (mid == 0) break;
      WORD fl = gpGlobals->g.rgObject[mid].magic.wFlags;
...
static void PAL_AIDecide(WORD pi) {
   WORD r = gpGlobals->rgParty[pi].wPlayerRole;
   WORD ch = gpGlobals->g.PlayerRoles.rgwHP[r];
   WORD cm = gpGlobals->g.PlayerRoles.rgwMaxHP[r];
   WORD mp = gpGlobals->g.PlayerRoles.rgwMP[r];
   WORD mm = gpGlobals->g.PlayerRoles.rgwMaxMP[r];

   // Count and assess enemies
   int enemyCount = 0, aliveEnemyCount = 0;
   int target = -1, strongTarget = -1;
   int minHP = 0xFFFF, maxEnemyHP = 0;
   DWORD totalEnemyHP = 0;
   for (int i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
      if (g_Battle.rgEnemy[i].wObjectID) {
         enemyCount++;
         if (g_Battle.rgEnemy[i].e.wHealth > 0) {
            aliveEnemyCount++;
            totalEnemyHP += g_Battle.rgEnemy[i].e.wHealth;
            if (g_Battle.rgEnemy[i].e.wHealth < minHP) { minHP = g_Battle.rgEnemy[i].e.wHealth; target = i; }
            if (g_Battle.rgEnemy[i].e.wHealth > maxEnemyHP) { maxEnemyHP = g_Battle.rgEnemy[i].e.wHealth; strongTarget = i; }
         }
      }
   }
   if (aliveEnemyCount == 0) {
      g_Battle.rgPlayer[pi].action.ActionType = kBattleActionPass;
      return;
   }

   // Assess ally states
   int healTarget = pi, aliveAllyCount = 0, deadTarget = -1;
   int lowestHP = PAL_AIGetAllyHealth(pi);
   int statusTarget = -1;
   int totalAllyHP = 0, totalAllyMaxHP = 0;

   for (int i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
      WORD rr = gpGlobals->rgParty[i].wPlayerRole;
      WORD ahp = gpGlobals->g.PlayerRoles.rgwHP[rr];
      WORD amx = gpGlobals->g.PlayerRoles.rgwMaxHP[rr];
      if (ahp == 0) { deadTarget = i; continue; }
      aliveAllyCount++;
      totalAllyHP += ahp;
      totalAllyMaxHP += amx;
      if (ahp < lowestHP) { lowestHP = ahp; healTarget = i; }
      if (statusTarget < 0 && (
         gpGlobals->rgPlayerStatus[rr][kStatusSleep] > 0 ||
         gpGlobals->rgPlayerStatus[rr][kStatusParalyzed] > 0 ||
         gpGlobals->rgPlayerStatus[rr][kStatusConfused] > 0 ||
         gpGlobals->rgPlayerStatus[rr][kStatusSilence] > 0)) {
         statusTarget = i;
      }
   }

   // Calculate key ratios
   WORD healTargetMaxHP = gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[healTarget].wPlayerRole];
   WORD healPct = (healTargetMaxHP > 0) ? lowestHP * 100 / healTargetMaxHP : 0;
   WORD selfHPPct = (cm > 0) ? ch * 100 / cm : 0;
   WORD mpPct = (mm > 0) ? mp * 100 / mm : 0;
   WORD partyHPPct = (totalAllyMaxHP > 0) ? totalAllyHP * 100 / totalAllyMaxHP : 0;

   int mpReserve = 10; // always keep at least 10 MP for emergencies
   BOOL hasOffensiveMagic = PAL_AIHasOffensiveMagic(r);
   BOOL hasHealMagic = PAL_AIHasHealMagic(r);

   // ===================================================================
   // DECISION TREE: Simulate human strategic thinking
   // ===================================================================

   // === SITUATION 1: Dead teammate ===
   if (deadTarget >= 0) {
      // Try revival item first (items like 还魂香 revive)
      WORD item = PAL_AIFindItem(TRUE);
      if (item) { PAL_AIActionUseItem(pi, item, deadTarget); return; }
      // Try healing magic (some magics can revive)
      int cost = 0;
      WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
      if (mid && mp >= cost + mpReserve) {
         PAL_AIActionMagic(pi, mid, deadTarget);
         return;
      }
      // Can't revive, just attack
   }

   // === SITUATION 2: Self has harmful status ===
   if (gpGlobals->rgPlayerStatus[r][kStatusSleep] > 0 ||
       gpGlobals->rgPlayerStatus[r][kStatusParalyzed] > 0 ||
       gpGlobals->rgPlayerStatus[r][kStatusConfused] > 0) {
      // Can't do anything useful, status will wear off
      PAL_AIActionAttack(pi, target); // will fail but that's OK
      return;
   }

   // === SITUATION 3: Ally has harmful status ===
   if (statusTarget >= 0 && statusTarget != (int)pi) {
      // Try curing magic
      int cost = 0;
      WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
      if (mid && mp >= cost + mpReserve) { PAL_AIActionMagic(pi, mid, statusTarget); return; }
      // Try item
      WORD item = PAL_AIFindItem(TRUE);
      if (item) { PAL_AIActionUseItem(pi, item, statusTarget); return; }
   }

   // === SITUATION 4: Self critically low HP (< 25%) ===
   if (selfHPPct < 25) {
      // Priority: self-preservation!
      int cost = 0;
      WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
      if (mid && mp >= cost) { PAL_AIActionMagic(pi, mid, pi); return; }
      // No MP, try item
      WORD item = PAL_AIFindItem(TRUE);
      if (item) { PAL_AIActionUseItem(pi, item, pi); return; }
      // Nothing available -> DEFEND to reduce damage
      PAL_AIActionDefend(pi);
      return;
   }

   // === SITUATION 5: Party HP critically low (< 35%) ===
   if (partyHPPct < 35) {
      // Check if the LOWEST ally needs urgent help
      if (healPct < 40) {
         int cost = 0;
         WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
         if (mid && mp >= cost + mpReserve) {
            // Use party heal if available and many are hurt
            WORD magicNum = gpGlobals->g.rgObject[mid].magic.wMagicNumber;
            WORD type = gpGlobals->g.lprgMagic[magicNum].wType;
            int aTarget = (type == kMagicTypeApplyToParty || aliveAllyCount <= 1) ? -1 : healTarget;
            PAL_AIActionMagic(pi, mid, aTarget);
            return;
         }
         // No MP for magic -> use item on most injured ally
         WORD item = PAL_AIFindItem(TRUE);
         if (item) { PAL_AIActionUseItem(pi, item, healTarget); return; }
      }
   }

   // === SITUATION 6: Ally HP low (40-60%) - consider healing ===
   if (healPct < 60 && healTarget != (int)pi) {
      int cost = 0;
      WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
      // Only heal with cheap magic or if we have plenty MP
      if (mid && mp >= cost + mpReserve && cost <= 15) {
         PAL_AIActionMagic(pi, mid, healTarget);
         return;
      }
      // Don't use items for moderate healing - save for emergencies
   }

   // === SITUATION 7: HP is fine, check self buff ===
   if (selfHPPct > 70 && mpPct > 50 && hasHealMagic) {
      // If silence, try to cure self
      if (gpGlobals->rgPlayerStatus[r][kStatusSilence] > 0) {
         int cost = 0;
         WORD mid = PAL_AISelectHealMagic(r, mp, &cost);
         if (mid && mp >= cost) { PAL_AIActionMagic(pi, mid, pi); return; }
      }
   ...
   // === SITUATION 8: Attack phase! ===
   // Decision: multi-target magic vs single-target magic vs items vs physical

   // 8a: If 3+ enemies alive, try AoE magic
   if (aliveEnemyCount >= 3 && mp >= 15) {
      int cost = 0;
      WORD mid = PAL_AISelectAttackMagic(r, mp, TRUE, &cost);
      if (mid && mp >= cost + mpReserve) {
         PAL_AIActionMagic(pi, mid, -1); // -1 = target all
         return;
      }
   }

   // 8b: Strong attack magic on primary target
   if (mp >= 10 && hasOffensiveMagic) {
      int cost = 0;
      WORD mid = PAL_AISelectAttackMagic(r, mp, FALSE, &cost);
      if (mid && mp >= cost + mpReserve) {
         PAL_AIActionMagic(pi, mid, target);
         return;
      }
   }

   // 8c: Single enemy left, low HP - finish with physical attack
   if (aliveEnemyCount == 1 && minHP < 100) {
      PAL_AIActionAttack(pi, target);
      return;
   }

   // 8d: Try throwable items at enemies
   WORD item = PAL_AIFindItem(FALSE);
   if (item) {
      g_Battle.rgPlayer[pi].action.ActionType = kBattleActionThrowItem;
      g_Battle.rgPlayer[pi].action.wActionID = item;
      g_Battle.rgPlayer[pi].action.sTarget = target;
      g_Battle.UI.iSelectedIndex = target;
      g_Battle.UI.wObjectID = item;
   ...
   // 8e: If MP is too low for anything and no items, physical attack
   // Target the weakest enemy to reduce their numbers
   PAL_AIActionAttack(pi, target);
}

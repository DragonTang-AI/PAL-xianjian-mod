...
typedef struct tagGLOBALVARS
{
   ...
   //
#define MAX_TRAVEL_LOCATIONS  48
   WORD             rgTravelScenesVisited[MAX_TRAVEL_LOCATIONS]; // visited scene numbers (0 = end)
   WORD             rgTravelLocScene[MAX_TRAVEL_LOCATIONS];      // scene number per teleport slot
   WORD             rgTravelLocX[MAX_TRAVEL_LOCATIONS];          // entry tile X per slot
   WORD             rgTravelLocY[MAX_TRAVEL_LOCATIONS];          // entry tile Y per slot
   //
   // Waypoint system
   //
#define MAX_WAYPOINTS  8
   WORD             rgWaypointScene[MAX_WAYPOINTS];   // scene number (0=empty)
   WORD             rgWaypointX[MAX_WAYPOINTS];       // tile X
   WORD             rgWaypointY[MAX_WAYPOINTS];       // tile Y
   ...
   // Minimap system
   //
   INT              iMinimapTileW;          // actual map width in tiles
   INT              iMinimapTileH;          // actual map height in tiles
   INT              iMinimapMapNum;         // current map number for explored cache
   BYTE             explored[MAP_MAX_TILES]; // explored tiles per map
   BOOL             fShowMinimap;           // TRUE when minimap overlay is shown
   BOOL             fFullscreenMap;         // TRUE when full-screen map is shown

   //
   // Touch map interaction signals (set by input.c, consumed by play.c)
   //
   BOOL             fMapTouchCurrent;       // set when minimap area is touched
   BOOL             fNoClip;
   BOOL             fAIAutoBattle;
   BOOL             fNoEncounter;
   BOOL             fMapReveal;
   BOOL             fAllItems;            // map reveal toggle           // no random encounters           // smart AI auto-battle              // walk through walls
   BOOL             fMapTouchWorld;         // set when M button is touched
   //
   // Auto-navigate state
   //
   BOOL             fAutoNavActive;         // TRUE when auto-navigating
   INT              iAutoNavTargetX;        // target tile X
   INT              iAutoNavTargetY;        // target tile Y
   INT              iAutoNavTargetH;        // target tile H
   //
   // Bestiary tracking
   //
#define MAX_BESTIARY_ENTRIES      128
   WORD             rgBestiary[MAX_BESTIARY_ENTRIES];   // encountered enemy object IDs (0 = empty)
   WORD             rgBestiaryKills[MAX_BESTIARY_ENTRIES]; // kills per entry
   INT              iBestiaryCount;                     // number of entries
   //
   // Minimap zoom / scroll for fullscreen mode
   //
   INT              iMapScrollX;            // scroll offset in fullscreen mode
   INT              iMapScrollY;
...
//
// Boss Rush scaling multiplier (percentage, 100 = 1x)
//
extern WORD g_wBossRushScale;

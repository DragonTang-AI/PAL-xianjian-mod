...
--*/
{
   ...
   //
   // Flying sword effect when NoClip is active
   //
   if (gpGlobals->fNoClip)
   {
      int _fx = PAL_X(gpGlobals->viewport) + PAL_X(gpGlobals->partyoffset);
      int _fy = PAL_Y(gpGlobals->viewport) + PAL_Y(gpGlobals->partyoffset);
      int _sx = _fx - PAL_X(gpGlobals->viewport);
      int _sy = _fy - PAL_Y(gpGlobals->viewport);
      
      // Only draw if on screen
      if (_sx > -16 && _sx < 336 && _sy > -16 && _sy < 216)
      {
         // Draw flying sword beneath the character (horizontal, 4x)
         SDL_Rect _s;
         // Blade body (horizontal, pointing right)
         _s.x = _sx - 4;  _s.y = _sy + 8;  _s.w = 38; _s.h = 6;
         SDL_FillRect(gpScreen, &_s, 0x2C);
         // Blade outline / edge highlight
         _s.x = _sx - 4;  _s.y = _sy + 8;  _s.w = 38; _s.h = 1;
         SDL_FillRect(gpScreen, &_s, 0x4F);
         // Sword tip (triangle pointing right)
         for (int _ti = 0; _ti < 5; _ti++) {
            _s.x = _sx + 34 + _ti;  _s.y = _sy + 10 - _ti;  _s.w = 1;  _s.h = 1 + _ti * 2;
            SDL_FillRect(gpScreen, &_s, 0x4A);
         }
         // Crossguard (vertical bar)
         _s.x = _sx - 6;  _s.y = _sy + 6;  _s.w = 3; _s.h = 10;
         SDL_FillRect(gpScreen, &_s, 0x1B);
         // Handle (to the left, wrapped)
         _s.x = _sx - 12;  _s.y = _sy + 8;  _s.w = 7; _s.h = 6;
         SDL_FillRect(gpScreen, &_s, 0x4F);
         // Handle wrapping lines
         _s.x = _sx - 10;  _s.y = _sy + 8;  _s.w = 1; _s.h = 6;
         SDL_FillRect(gpScreen, &_s, 0x44);
         _s.x = _sx - 7;   _s.y = _sy + 8;  _s.w = 1; _s.h = 6;
         SDL_FillRect(gpScreen, &_s, 0x44);
         // Glow aura around the blade
         _s.x = _sx - 2;  _s.y = _sy + 6;  _s.w = 34; _s.h = 10;
         SDL_FillRect(gpScreen, &_s, 0x1B);
         // Inner glow
         _s.x = _sx - 1;  _s.y = _sy + 9;  _s.w = 30; _s.h = 4;
         SDL_FillRect(gpScreen, &_s, 0x2C);
      }
   }
...
--*/
{
   if (gpGlobals && gpGlobals->fNoClip) return FALSE;
   int x, y, h, xr, yr;

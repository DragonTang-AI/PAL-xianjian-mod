...
--*/
{
   ...
   if (gpGlobals->fAutoBattle)
   {
      ...
      //
      // Draw player info boxes during auto-battle
      //
      for (int _ai = 0; _ai <= gpGlobals->wMaxPartyMemberIndex; _ai++) {
         WORD _wr = gpGlobals->rgParty[_ai].wPlayerRole;
         PAL_PlayerInfoBox(PAL_XY(91 + 77 * _ai, 165), _wr,
            (WORD)(g_Battle.rgPlayer[_ai].flTimeMeter), TIMEMETER_COLOR_DEFAULT, FALSE);
      }
...
//         g_Battle.UI.iPrevEnemyTarget = g_Battle.UI.iSelectedIndex; //disabled due to not same as both original version
   ...
   //
   // Draw the numbers with enhanced animation
   //
   for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++)
   {
      if (g_Battle.UI.rgShowNum[i].wNum > 0)
      {
         int _elapsed = (SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / BATTLE_FRAME_TIME;
         int _maxFrames = 18; // show for ~18 frames (0.9s at 20fps)

         if (_elapsed > _maxFrames)
         {
            g_Battle.UI.rgShowNum[i].wNum = 0;
         }
         else
         {
            int _px = PAL_X(g_Battle.UI.rgShowNum[i].pos) + g_Battle.UI.rgShowNum[i].sOffsetX;
            int _py = PAL_Y(g_Battle.UI.rgShowNum[i].pos) - _elapsed * 2; // float up faster

            // Fade out: change color based on remaining time
            NUMCOLOR _drawColor = g_Battle.UI.rgShowNum[i].color;
            if (_elapsed > _maxFrames - 4) {
               // In last 4 frames, swap to darker color for fade effect
               if (_drawColor == kNumColorYellow) _drawColor = kNumColorCyan;
            }

            // Draw sign prefix for healing numbers (yellow)
            if (g_Battle.UI.rgShowNum[i].color == kNumColorYellow)
            {
               PAL_DrawText(L"+", PAL_XY(_px - 14, _py), 0x2C, FALSE, FALSE, TRUE);
            }

            PAL_DrawNumber(g_Battle.UI.rgShowNum[i].wNum, 4,
               PAL_XY(_px, _py),
               _drawColor, kNumAlignRight);
         }
      }
   }
   ...
   //
    // Enhanced Battle HUD (AI auto-battle mode)
    //
    if (gpGlobals->fInBattle && gpGlobals->fAIAutoBattle) {
       // TOP: Enemy name + HP bar + HP numbers
       int _ec = 0;
       for (int _ei = 0; _ei <= g_Battle.wMaxEnemyIndex; _ei++) {
          if (g_Battle.rgEnemy[_ei].wObjectID != 0) {
             WORD _ch = g_Battle.rgEnemy[_ei].e.wHealth;
             WORD _mh = g_Battle.rgEnemy[_ei].wTrueMaxHP;
             if (_mh == 0) { _mh = _ch; if (_mh < 1) _mh = 1; }
             if (_ch > _mh) _ch = _mh;
             int _bx = 4 + _ec * 78;
             // Enemy name
             PAL_DrawText(PAL_GetWord(g_Battle.rgEnemy[_ei].wObjectID), PAL_XY(_bx, 0), 0x2C, FALSE, FALSE, FALSE);
             // HP bar background
             SDL_Rect _br; _br.x=_bx; _br.y=16; _br.w=74; _br.h=6; SDL_FillRect(gpScreen, &_br, 0x00);
             _br.x=_bx+1; _br.y=17; _br.w=72; _br.h=4; SDL_FillRect(gpScreen, &_br, 0x11);
             // HP bar fill
             int _fw = (_mh > 0) ? _ch * 70 / _mh : 0;
             if (_fw > 0) { _br.x=_bx+2; _br.y=18; _br.w=_fw; _br.h=2;
                SDL_FillRect(gpScreen, &_br, _ch > _mh/2 ? 0x2C : (_ch > _mh/4 ? 0x5B : 0x4A)); }
             // HP numbers
             PAL_DrawNumber(_ch, 4, PAL_XY(_bx+4, 24), kNumColorYellow, kNumAlignLeft);
             PAL_DrawText(L"/", PAL_XY(_bx+36, 24), 0x1C, FALSE, FALSE, FALSE);
             PAL_DrawNumber(_mh, 4, PAL_XY(_bx+42, 24), kNumColorCyan, kNumAlignLeft);
             _ec++;
          }
       }
       // BOTTOM: Player name + HP bar + MP bar
       for (int _pi = 0; _pi <= gpGlobals->wMaxPartyMemberIndex; _pi++) {
          WORD _pr = gpGlobals->rgParty[_pi].wPlayerRole;
          WORD _hp = gpGlobals->g.PlayerRoles.rgwHP[_pr];
          WORD _hm = gpGlobals->g.PlayerRoles.rgwMaxHP[_pr];
          WORD _mp = gpGlobals->g.PlayerRoles.rgwMP[_pr];
          WORD _mm = gpGlobals->g.PlayerRoles.rgwMaxMP[_pr];
          int _px = 4 + _pi * 78, _py = 152;
          // Player name
          PAL_DrawText(PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[_pr]), PAL_XY(_px, _py-10), 0x4F, FALSE, FALSE, FALSE);
          // HP bar
          SDL_Rect _hr; _hr.x=_px; _hr.y=_py; _hr.w=72; _hr.h=5; SDL_FillRect(gpScreen, &_hr, 0x00);
          _hr.x=_px+1; _hr.y=_py+1; _hr.w=70; _hr.h=3; SDL_FillRect(gpScreen, &_hr, 0x11);
          if (_hm > 0) { int _hw = _hp * 68 / _hm; if (_hw > 0) { _hr.x=_px+2; _hr.y=_py+2; _hr.w=_hw; _hr.h=1; SDL_FillRect(gpScreen, &_hr, 0x2C); } }
          PAL_DrawNumber(_hp, 4, PAL_XY(_px+2, _py+6), kNumColorYellow, kNumAlignLeft);
          PAL_DrawText(L"/", PAL_XY(_px+34, _py+6), 0x1C, FALSE, FALSE, FALSE);
          PAL_DrawNumber(_hm, 4, PAL_XY(_px+40, _py+6), kNumColorCyan, kNumAlignLeft);
          // MP bar
          _hr.x=_px; _hr.y=_py+14; _hr.w=72; _hr.h=5; SDL_FillRect(gpScreen, &_hr, 0x00);
          _hr.x=_px+1; _hr.y=_py+15; _hr.w=70; _hr.h=3; SDL_FillRect(gpScreen, &_hr, 0x11);
          if (_mm > 0) { int _mw = _mp * 68 / _mm; if (_mw > 0) { _hr.x=_px+2; _hr.y=_py+16; _hr.w=_mw; _hr.h=1; SDL_FillRect(gpScreen, &_hr, 0x1B); } }
          PAL_DrawNumber(_mp, 4, PAL_XY(_px+2, _py+20), kNumColorYellow, kNumAlignLeft);
          PAL_DrawText(L"/", PAL_XY(_px+34, _py+20), 0x1C, FALSE, FALSE, FALSE);
          PAL_DrawNumber(_mm, 4, PAL_XY(_px+40, _py+20), kNumColorCyan, kNumAlignLeft);
       }
    }

   if (gpGlobals->fAIAutoBattle)
   {
      PAL_DrawText(L"[AI Battle]", PAL_XY(80, 2), 0x2C, TRUE, FALSE, TRUE);
   }
}
...
--*/
{
   ...
   for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++)
   {
      if (g_Battle.UI.rgShowNum[i].wNum == 0)
      {
         ...
         // Random horizontal spread to prevent stacking (-6 to +6)
         g_Battle.UI.rgShowNum[i].sOffsetX = (SHORT)((SDL_GetTicks() % 13) - 6);
